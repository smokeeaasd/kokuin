...
CI runs: typecheck → lint → format:check → test → build.

## Code conventions

- **Single entrypoint**: `src/index.ts` exports a `hash(value: unknown): string` function. That's the entire public API — no `serialize`, no `digest`, no algorithm options.
- **No semicolons**, single quotes, no trailing commas, 100 print width (enforced by `oxfmt`).
- **No `forEach`** — use `for...of`. No bitwise operators. No `var`. Prefer arrow functions. `console` is warn-level. `eqeqeq` always (no `==` null checks).
...
- Internally uses SHA-256 via `crypto.subtle` (Web) / `crypto.createHash` (Node) — zero dependencies, built-in to all runtimes.
...
- Tests use Vitest with `globals: true` (no explicit imports needed for `describe`/`it`/`expect`).
- `lint-staged` runs `oxlint --fix && oxfmt --write` on staged `*.ts` files.
- Release: push tag `v*` → `pnpm build && pnpm publish --no-git-checks --provenance`.
...
- Depth-limited: excessive nesting must throw.